...
describe('Testing Posts', () => {
  describe('[GET] /posts/:groupId', () => {
    ...
    it('response statusCode 200 / findByGroupId', async () => {
      ...
      return await request(app.getServer()).get(`${postsRoute.path}/${groupId}`).auth(token, { type: 'bearer' }).expect(200);
    });
    it('response statusCode 403 / forbidden', async () => {
      ...
      return await request(app.getServer()).get(`${postsRoute.path}/${groupId}`).auth(token, { type: 'bearer' }).expect(403);
    });
  });
  ...
  describe('[POST] /posts', () => {
    ...
    it('response statusCode 201 / created', async () => {
      ...
      return await request(app.getServer()).post(`${postsRoute.path}`).auth(token, { type: 'bearer' }).send(postData).expect(201);
    });
    it('response statusCode 403 / forbidden', async () => {
      const postData: CreatePostDto = {
        groupId: 101,
      ...
      return await request(app.getServer()).post(`${postsRoute.path}`).auth(token, { type: 'bearer' }).send(postData).expect(403);
    });
    it('response statusCode 400 / bad request', async () => {
      ...
      return await request(app.getServer()).post(`${postsRoute.path}`).auth(token, { type: 'bearer' }).send(postData).expect(400);
    });
  });
});
